...
XRPC has a `CompatClient` that is compatible with Indigo's LexClient.
You can wrap any `Client` created with XRPC as a `CompatClient` to use them in lexicons generated by Indigo.

## Examples

If you want real examples, you can look at:
- [Lasa](https://tangled.org/anhgelus.world/lasa), a stateless proxy that generates a RSS or an Atom feed from a Standard.site publication.
- [GoAT Site](https://tangled.org/anhgelus.world/goat-site), a library that implements Standard.site in Go.
